...
class EasySet(Dataset):
    ...
    def compose_transforms(image_size: int, training: bool) -> transforms.Compose:
        ...
        return (
            transforms.Compose(
                [
                    transforms.Resize([int(image_size * 1.10), int(image_size * 1.10)], interpolation=BICUBIC),
                    transforms.RandomResizedCrop(image_size),
                    transforms.RandomHorizontalFlip(),
                    transforms.ColorJitter(
                        brightness=0.2 * random.uniform(0,1),
                        contrast=0.2 * random.uniform(0,1),
                        saturation=0.2 * random.uniform(0,1),
                        hue=0),
            ...
            if training
                ...
                [
                    transforms.Resize([int(image_size), int(image_size)], interpolation=BICUBIC),
